...
// Bump-allocating provider over a fixed Vec. `free_memory` is a no-op, and
// `init` raises the advertised alignment guarantee when the pool requires it.
struct BumpProvider {
    buffer: Vec<u8>,
    offset: usize,
    alignment: usize,
...
unsafe impl MemoryProvider for BumpProvider {
...
// Over-aligned object that forces the pool to request 64-byte-aligned slabs.
#[repr(C, align(64))]
struct HardwareTask {
    id: u32,
}
...
fn slab_allocator_aligns_header_even_for_byte_slots() {
    let mut provider = BumpProvider::new(1024, 1);
    let mut allocator =
...
fn pool_drop_allows_balanced_raw_slots() {
    let mut provider = BumpProvider::new(4096, 64);
    let mut pool = Pool::<Task, _>::new_uninit(&mut provider, 1).unwrap();
    pool.init();
...
fn pool_provider_exhaustion_still_reuses_returned_slot() {
    ...
    let mut provider = BumpProvider::new(slab_size + 7, 8);
...
#[test]
fn pool_reuses_freed_slot_then_grows_a_new_slab() {
    let mut provider = BumpProvider::new(16384, 4096);
    let mut pool = Pool::<Task, _>::new_uninit(&mut provider, 2).unwrap();
...
#[test]
fn pool_honors_64_byte_object_alignment() {
    let mut provider = BumpProvider::new(4096, 64);
...
fn slab_does_not_allocate_from_alignment_padding() {
    let mut provider = BumpProvider::new(12288, 4096);
    let mut pool = Pool::<Task, _>::new_uninit(&mut provider, 1).unwrap();
    pool.init();
...
#[test]
fn pool_escalates_provider_alignment_for_overaligned_objects() {
    // Provider only guarantees 8-byte alignment
    let mut provider = BumpProvider::new(8192, 8);
